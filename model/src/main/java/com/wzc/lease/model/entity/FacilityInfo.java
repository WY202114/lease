...
@Schema(description = "配套信息表") // OpenAPI 注解，描述模型或字段含义
@TableName(value = "facility_info") // MyBatis-Plus 注解，指定实体对应的数据表
@Data // Lombok 注解，自动生成 getter/setter/toString 等方法
public class FacilityInfo extends BaseEntity { // 定义实体类
    private static final long serialVersionUID = 1L; // 序列化版本号，用于反序列化兼容性控制

    @Schema(description = "配套所属对象类型") // OpenAPI 注解，描述模型或字段含义
    @TableField(value = "type") // MyBatis-Plus 注解，指定实体字段对应的数据库字段
    private ItemType type; // 字段定义，保存实体属性

    @Schema(description = "名称") // OpenAPI 注解，描述模型或字段含义
    @TableField(value = "name") // MyBatis-Plus 注解，指定实体字段对应的数据库字段
    private String name; // 字段定义，保存实体属性

    @Schema(description = "图标") // OpenAPI 注解，描述模型或字段含义
    @TableField(value = "icon") // MyBatis-Plus 注解，指定实体字段对应的数据库字段
    private String icon; // 字段定义，保存实体属性
