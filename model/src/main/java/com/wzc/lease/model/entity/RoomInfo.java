...
import com.atguigu.lease.model.enums.ReleaseStatus; // 导入当前文件依赖的类型
...
import java.util.List; // 导入列表集合类型

@Schema(description = "房间信息表") // OpenAPI 注解，描述模型或字段含义
@TableName(value = "room_info") // MyBatis-Plus 注解，指定实体对应的数据表
@Data // Lombok 注解，自动生成 getter/setter/toString 等方法
public class RoomInfo extends BaseEntity { // 定义实体类

    private static final long serialVersionUID = 1L; // 序列化版本号，用于反序列化兼容性控制

    @Schema(description = "房间号") // OpenAPI 注解，描述模型或字段含义
    @TableField(value = "room_number") // MyBatis-Plus 注解，指定实体字段对应的数据库字段
    private String roomNumber; // 字段定义，保存实体属性
    ...
    @Schema(description = "所属公寓id") // OpenAPI 注解，描述模型或字段含义
