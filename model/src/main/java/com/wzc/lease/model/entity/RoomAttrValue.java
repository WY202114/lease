...
@Schema(description = "房间&基本属性值关联表") // OpenAPI 注解，描述模型或字段含义
@TableName(value = "room_attr_value") // MyBatis-Plus 注解，指定实体对应的数据表
@Data // Lombok 注解，自动生成 getter/setter/toString 等方法
@Builder // Lombok 注解，支持使用构建者方式创建对象
public class RoomAttrValue extends BaseEntity { // 定义实体类
    ...
    @Schema(description = "属性值id") // OpenAPI 注解，描述模型或字段含义
    @TableField(value = "attr_value_id") // MyBatis-Plus 注解，指定实体字段对应的数据库字段
    private Long attrValueId; // 字段定义，保存实体属性
} // 代码块结束
