...
@Schema(description = "房间基本属性值表") // OpenAPI 注解，描述模型或字段含义
@TableName(value = "attr_value") // MyBatis-Plus 注解，指定实体对应的数据表
@Data // Lombok 注解，自动生成 getter/setter/toString 等方法
public class AttrValue extends BaseEntity { // 定义实体类

    private static final long serialVersionUID = 1L; // 序列化版本号，用于反序列化兼容性控制

    @Schema(description = "属性value") // OpenAPI 注解，描述模型或字段含义
    @TableField(value = "name") // MyBatis-Plus 注解，指定实体字段对应的数据库字段
    private String name; // 字段定义，保存实体属性

    @Schema(description = "对应的属性key_id") // OpenAPI 注解，描述模型或字段含义
    @TableField(value = "attr_key_id") // MyBatis-Plus 注解，指定实体字段对应的数据库字段
    private Long attrKeyId; // 字段定义，保存实体属性
} // 代码块结束
