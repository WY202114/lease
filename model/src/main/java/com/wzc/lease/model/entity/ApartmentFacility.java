...
@Schema(description = "公寓&配套关系") // OpenAPI 注解，描述模型或字段含义
@TableName(value = "apartment_facility") // MyBatis-Plus 注解，指定实体对应的数据表
@Data // Lombok 注解，自动生成 getter/setter/toString 等方法
@NoArgsConstructor  // 加这个，补回public无参构造
@AllArgsConstructor // 加这个，配合@Builder使用
@Builder // Lombok 注解，支持使用构建者方式创建对象
public class ApartmentFacility extends BaseEntity { // 定义实体类
    ...
    @Schema(description = "设施id") // OpenAPI 注解，描述模型或字段含义
    @TableField(value = "facility_id") // MyBatis-Plus 注解，指定实体字段对应的数据库字段
    private Long facilityId; // 字段定义，保存实体属性
